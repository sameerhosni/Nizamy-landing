...
export function Calculator({
  ...
  const content = {
    en: {
      ...
      titlePart2: "Return",
      subhead:
        "Move the slider to see a sample estimate — actual results vary based on your team's performance.",
      ...
      results: {
        ...
        sampleNote:
          "Illustrative only — actual return depends on usage & performance",
      ...
      disclaimer:
        "These numbers are for illustration purposes only. Your actual return will vary depending on your team size, performance levels, and how effectively you use Nizamy HR.",
    },
    ar: {
      titlePart1: "قدّر ",
      titlePart2: "عائدك",
      subhead:
        "حرّك المؤشر لترى تقديراً توضيحياً — النتائج الفعلية تتفاوت حسب أداء فريقك.",
      ...
      disclaimer:
        "هذه الأرقام توضيحية فقط. عائدك الفعلي يتفاوت حسب حجم فريقك ومستويات الأداء ومدى فاعلية استخدامك لنظامي اتش آر.",
    },
  };
  ...
  const closingContent = {
    en: {
      ...
      footnote:
        "No fine print. Your team's growth genuinely benefits us too — that's the whole idea.",
    },
    ar: {
      quote: "لا نستفيد إلا حين تستفيد أنت.",
      footnote:
        "بدون شروط خفية. نمو فريقك يفيدنا نحن أيضاً بشكل حقيقي — هذه هي الفكرة كلها.",
    },
  };
  ...
  return (
    <section
      id="calculator"
      className="py-24 sm:py-32 bg-slate-50 scroll-mt-16"
    >
      <div className="container mx-auto px-4 sm:px-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <span className="text-4xl font-heading font-black text-blue-600 tabular-nums">
                  {employees}
                </span>
              </div>
              <Slider
                ...
                onValueCommit={(vals) =>
                  posthog.capture("calculator_employees_adjusted", {
                    employees: vals[0],
                    tier,
                    language,
                  })
            ...
            {/* Tier selector */}
            <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm">
              <label className="text-base font-bold block text-slate-800 mb-4">
                {t.tierLabel}
              </label>
              <div className="grid grid-cols-3 gap-3">
                {(["Standard", "Growth", "Pro"] as const).map((tOption) => (
                  <button
                    key={tOption}
                    onClick={() => {
                      posthog.capture("calculator_tier_selected", {
                        tier: tOption,
                        employees,
                        language,
                      });
            ...
            <div className="rounded-2xl bg-slate-900 overflow-hidden shadow-2xl">
              <div className="p-8 sm:p-10 space-y-6 relative">
                ...
                <div className="relative text-center pb-8 border-b border-white/10">
                  ...
                  <p className="text-[11px] font-medium text-slate-500 mt-3 bg-white/5 inline-block px-3 py-1 rounded-full">
                ...
                <div className="relative space-y-3">
                  <div className="flex justify-between items-center bg-white/5 p-4 rounded-xl">
                    <span className="text-slate-300 font-medium text-sm">
                      {t.results.subscription}
                    </span>
                    <span className="font-heading text-lg font-bold text-slate-200 tabular-nums">
                      {formatCurrency(subscription, "SAR", language)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center bg-blue-600/15 p-4 rounded-xl border border-blue-500/20">
                    ...
                    <span className="font-heading text-xl font-black text-white tabular-nums">
                      {formatCurrency(effectiveCost, "SAR", language)}
                    </span>
                  </div>
                </div>
                    ...
                    <div key={idx} className="space-y-2">
                      <div className="flex justify-between items-center text-xs font-medium text-slate-400">
                        ...
                        <span className="text-slate-200 font-semibold">
                          {formatCurrency(row.value, "SAR", language)}
                        </span>
                      </div>
        ...
        <div className="max-w-2xl mx-auto text-center mt-20 pt-12 border-t border-slate-200">
          ...
          <p className="text-2xl sm:text-3xl font-heading font-black leading-snug text-slate-900 mb-3">
            {ct.quote}
          </p>
