...
- Email HTML is table-based with fully inline styles (Gmail-safe); uses landing-page fonts (IBM Plex Sans Arabic body; Cairo AR / Tajawal EN headings via Google Fonts link+@import in head, with system fallbacks for clients that block web fonts) and brand palette (blue #2563EB, navy #0F172A); logo embedded as CID attachment from `artifacts/api-server/assets/logo-mascot.png` (resolved relative to the built module, graceful text-only fallback if missing)
- Because Gmail strips web fonts, display headlines (welcome headline + blue banner; returning headline + "up to 30%" stat) are pre-rendered PNGs in true Cairo/Tajawal ExtraBold at 2x retina, stored in `artifacts/api-server/assets/email-text/` (8 files: {welcome-headline,banner,returning-headline,stat}-{ar,en}.png) and attached as CID images per language; if any file is missing the mailer logs a warning and falls back to HTML text headlines. Regenerate with `pnpm --filter @workspace/scripts run generate-email-text-images` (needs ImageMagick+Pango and Cairo/Tajawal ExtraBold TTFs in `~/.fonts`); if dimensions change, update the hardcoded half-width `<img>` widths in `emailTemplates.ts`
