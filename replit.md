...
- Lead flow: form POST `/api/lead` → saved to Postgres → bilingual welcome email sent to the lead's own address + internal notification email to sales@nizamy.app with all submitted fields (name, company, email, WhatsApp, employees, tier, price, subscription, estimated return, language, new/returning badge; reply-to = lead's email); team email still sends even if the client email fails (templates in `artifacts/api-server/src/lib/emailTemplates.ts`, sender in `src/lib/mailer.ts`)
...
- Custom welcome template: if `artifacts/api-server/assets/email-custom/welcome-<lang>.html` (+ `-header.png`/`-footer.png`) exists, NEW leads in that language get it instead of the coded templates (currently AR only, user-provided Cairo design). Placeholders `{{first_name}}` (HTML-escaped), `{{cta_url}}`=TRIAL_LINK, `{{website_url}}`, `{{unsubscribe_url}}`; subject read from `<title>`. Images were converted from data URIs to CID attachments and the ~240KB base64 @font-face blocks stripped (Gmail strips both and clips emails >102KB). Returning leads and EN keep the old templates; drop in `welcome-en.html` + PNGs to extend.
